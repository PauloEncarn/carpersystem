...
import { loadActiveRg003Cycle, loadRg003CyclesByDate, makeRg003ProductionCode, persistCycleNc, persistCycleTransition, startRg003Cycle } from "@/lib/rg003Persistence";
...
function Rg003ProductionControl({ lineId, dateId, operatorId, operatorName, onOpenProcess }) {
  const storageKey = `carper_rg003_cycle_${lineId}`;
  const [cycle, setCycle] = useState(null);
  ...
  useEffect(() => {
    let active = true;
    loadRg003CyclesByDate(lineId, dateId).then((result) => { if (active) setDayCycles(result.cycles); }).catch(() => { if (active) setDayCycles([]); });
    return () => { active = false; };
  ...
  async function prepare(reason = "Início de produção", selectedProduct = product) {
    const at = new Date().toISOString();
    let next = { id: `CICLO-${Date.now()}`, productionCode: makeRg003ProductionCode(selectedProduct, at), product: selectedProduct, previousProduct: cycle?.product ?? "", reason, status: "hygiene", startedAt: at, stageStartedAt: at, events: [localEvent("Preparação iniciada")] };
    setSyncState("saving");
  ...
  const canCancelPreparation = Boolean(cycle) && ["hygiene", "awaiting_release", "ready"].includes(cycle.status);
  ...
  const products = ["Rosca Leite", "Rosca Coco", "Rosca Chocolate", "Rosca Tradicional"];
  const selectedDayReference = dateId ? new Date(`${dateId}T12:00:00-03:00`) : new Date();

  return <div className="mx-auto max-w-6xl space-y-4"><section className="rounded-lg border border-gray-300 border-t-4 border-t-cicopal-blue bg-white p-5"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-bold uppercase text-cicopal-blue">RG.QUA.BA.003 · Rosca</p><h2 className="mt-1 text-2xl font-bold text-gray-950">{cycle?.product ?? "Nova produção"}</h2>{cycle ? <p className="mt-1 font-mono text-sm font-bold text-gray-600">{cycle.productionCode || makeRg003ProductionCode(cycle.product, cycle.startedAt)}</p> : null}<p className={`mt-2 text-sm font-bold ${producing ? "text-cicopal-green" : cycle?.status === "blocked" ? "text-cicopal-red" : "text-amber-700"}`}>{statusText}</p><p className={`mt-2 inline-flex items-center gap-2 text-xs font-bold ${syncState === "online" ? "text-cicopal-green" : syncState === "error" ? "text-cicopal-red" : "text-gray-500"}`}><span className={`size-2 rounded-full ${syncState === "online" ? "bg-cicopal-green" : syncState === "saving" || syncState === "checking" ? "animate-pulse bg-amber-500" : syncState === "error" ? "bg-cicopal-red" : "bg-gray-400"}`} />{syncText}</p></div><div className="text-right"><p className="text-xs font-bold uppercase text-gray-400">Tempo de produção</p><p className="text-3xl font-bold tabular-nums text-gray-950">{formatElapsed(cycle?.productionStartedAt, now)}</p></div></div></section>

  {!cycle ? <section className="rounded-lg border border-gray-300 bg-white p-5"><div className="mb-4"><p className="text-xs font-bold uppercase text-gray-500">Produtos do dia</p><h3 className="mt-1 text-xl font-bold text-gray-950">Escolha o produto da nova produção</h3><p className="mt-1 text-sm font-semibold text-gray-600">Nenhum produto vem pré-selecionado. Toque no cartão correto antes de continuar.</p></div><div className="grid gap-3 sm:grid-cols-2">{products.map((item) => { const productions = dayCycles.filter((entry) => entry.product === item); const selectedProduct = product === item; return <button key={item} type="button" onClick={() => setProduct(item)} className={`min-h-32 rounded-md border-2 p-4 text-left ${selectedProduct ? "border-cicopal-blue bg-blue-50 ring-2 ring-blue-100" : "border-gray-200 bg-white"}`}><span className="block text-lg font-bold text-gray-950">{selectedProduct ? "✓ " : ""}{item}</span><span className="mt-1 block text-sm font-semibold text-gray-500">{productions.length ? `${productions.length} produção(ões) iniciada(s)` : "Nenhuma produção neste dia"}</span>{productions.map((entry) => <span key={entry.id} className="mt-2 block rounded bg-white px-2 py-1 text-xs font-bold text-cicopal-blue">{entry.productionCode || makeRg003ProductionCode(entry.product, entry.startedAt)} · {entry.status === "ended" ? "Encerrada" : "Em andamento"}</span>)}</button>; })}</div><div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 pt-4"><div><p className="text-xs font-bold uppercase text-gray-500">Código previsto</p><p className="font-bold text-gray-950">{product ? makeRg003ProductionCode(product, selectedDayReference) : "Selecione um produto"}</p></div><button type="button" disabled={!product} className="min-h-16 rounded-md bg-cicopal-blue px-6 text-lg font-bold text-white disabled:bg-gray-300 disabled:text-gray-600" onClick={() => prepare()}>{product ? `Criar produção de ${product.replace(/^Rosca /, "")}` : "Selecione um produto"}</button></div></section> : <><section className="rounded-lg border border-gray-300 bg-white p-4"><p className="mb-3 text-xs font-bold uppercase text-gray-500">1 · Pré-requisitos · {cycle.productionCode || makeRg003ProductionCode(cycle.product, cycle.startedAt)}</p><div className="grid gap-3 md:grid-cols-2"><button type="button" onClick={() => onOpenProcess("higienizacao")} className={`min-h-24 rounded-md border-2 p-4 text-left ${hygieneDone ? "border-green-300 bg-green-50 text-cicopal-green" : "border-cicopal-blue bg-blue-50 text-cicopal-blue"}`}><span className="block text-lg font-bold">{hygieneDone ? "✓ Higienização confirmada" : "Higienização"}</span><span className="mt-1 block text-sm font-semibold">{hygieneDone ? "Registro único · toque para visualizar" : "Registro único obrigatório antes de iniciar"}</span></button><button type="button" disabled={!hygieneDone} onClick={() => onOpenProcess("produto_liberacao")} className={`min-h-24 rounded-md border-2 p-4 text-left ${releaseDone ? "border-green-300 bg-green-50 text-cicopal-green" : hygieneDone ? "border-cicopal-blue bg-blue-50 text-cicopal-blue" : "border-gray-200 bg-gray-100 text-gray-400"}`}><span className="block text-lg font-bold">{releaseDone ? "✓ Produto liberado" : "Liberação do produto"}</span><span className="mt-1 block text-sm font-semibold">{releaseDone ? "Registro único · toque para visualizar" : hygieneDone ? "Um registro para toda a produção" : "Aguardando higienização"}</span></button></div></section>

  <section className="rounded-lg border border-gray-300 bg-white p-5"><p className="text-center text-xs font-bold uppercase text-gray-500">2 · Controle da produção</p><button type="button" disabled={!releaseDone && !producing} onClick={() => producing ? setStopOpen(true) : transition("producing", "Produção iniciada", { productionStartedAt: new Date().toISOString(), activeAction: null })} className={`mx-auto mt-4 grid size-28 place-items-center rounded-full border-8 text-white shadow-md transition ${producing ? "border-red-100 bg-cicopal-red" : releaseDone ? "border-blue-100 bg-cicopal-blue" : "border-gray-200 bg-gray-300"}`}>{producing ? <Square size={38} fill="currentColor" /> : <Play size={42} fill="currentColor" className="ml-1" />}</button><p className="mt-3 text-center text-lg font-bold text-gray-900">{producing ? "Parar produção" : releaseDone ? "Iniciar produção" : "Aguardando pré-requisitos"}</p>{canCancelPreparation ? <button type="button" className="mx-auto mt-4 flex min-h-12 items-center justify-center gap-2 rounded-md border border-gray-300 bg-white px-5 font-bold text-gray-700" onClick={() => { setStopMode("cancel"); setStopOpen(true); }}><ArrowLeft size={18} /> Voltar aos produtos</button> : <p className="mt-4 text-center text-xs font-semibold text-gray-500">Com a produção iniciada, utilize o botão vermelho para parar.</p>}</section>
...
export function HierarchyNavigator({ tree, selection, selected, onSelectionChange, currentStep, onStepChange, children, hideDates = false, operatorName = "", operatorId = "" }) {
  ...
  function abrirRegistroTecnico(processoId) {
    ...
    let productionCode = "";
    try { const activeCycle = JSON.parse(window.localStorage.getItem(`carper_rg003_cycle_${selection.linhaId}`) ?? "null"); activeCycleId = activeCycle?.id ?? activeCycleId; productionCode = activeCycle?.productionCode ?? ""; } catch { /* usa identificador local */ }
    ...
    onSelectionChange({ ...selection, subregistroId: processoId, registroId: existing?.id ?? (productionCode ? `${productionCode}-${prefix}` : `${rgPrefix}-${selection.loteId}-${cycleSuffix}-${prefix}`) });
    onStepChange(6);
  }
