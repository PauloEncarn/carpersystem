...
const guidedFieldTypes = [
  ...
  { id: "separador", label: "Separador visual", visual: true },
...
export function GuidedRgBuilder({ lines, onCancel, onCreate }) {
  ...
  function addVisualField(sectionId, type) {
    const defaults = {
      titulo: "Novo título",
      instrucao: "Adicione aqui uma orientação importante para o operador.",
      separador: ""
    };
    ...
    <section className="rounded-[20px] border border-gray-200/80 bg-[#f5f7fc] p-3 shadow-soft">
      ...
      <div className="mb-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-sm">
        <div className="min-w-0">
          <p className="truncate text-sm font-black text-gray-950">{draft.code} — {draft.title}</p>
          <p className="text-xs font-semibold text-gray-500">{draft.sections.length} seções • {fields.length} elementos • revisão {draft.revision}</p>
        </div>
        <div className="flex flex-wrap gap-1.5">
          <button type="button" className="min-h-9 rounded-lg border border-gray-200 bg-gray-50 px-3 text-xs font-bold text-gray-600" onClick={() => goToStep("basico")}>Editar dados</button>
          <button type="button" className="min-h-9 rounded-lg border border-gray-200 bg-gray-50 px-3 text-xs font-bold text-gray-600" onClick={() => goToStep("estrutura")}>Editar estrutura</button>
          <button type="button" className="min-h-9 rounded-lg border border-gray-200 bg-gray-50 px-3 text-xs font-bold text-gray-600" onClick={() => goToStep("layout")}>Ajustar layout</button>
        </div>
      </div>
