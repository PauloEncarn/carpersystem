...
export function ProductionProcessFlow({ cycle, operatorId }) {
  ...
  return <section className="border border-gray-300 bg-white p-4">
    ...
    <div className="mt-5 flex gap-3 overflow-x-auto pb-3">{rows.map((process, index) => { const cfg = ROSCA_SUBPROCESSES.find((item) => item.code === process.codigo); const latest = records.filter((item) => item.subprocesso_id === process.id).at(-1); return <div key={process.id} className="flex min-w-[230px] items-center gap-3"><button onClick={() => { setSelectedCode(process.codigo); setMessage(""); }} className={`min-h-44 w-full border-2 p-4 text-left ${tones[process.status]}`}><small className="font-black uppercase">{cfg?.frequency === "production" ? "Rastreabilidade" : "Controle 60 min"}</small><strong className="mt-2 block text-xl">{process.nome}</strong><span className="block font-bold text-gray-600">{process.equipamento}</span><b className="mt-4 block uppercase">{labels[process.status]}</b><span className="font-mono text-lg font-black">{elapsed(process.estado_iniciado_em, now)}</span>{latest ? <span className="mt-2 block text-xs font-bold text-green-700">Último dado · {formatTime(latest.preenchido_em ?? latest.horario_referencia)}</span> : null}</button>{index < rows.length - 1 ? <b className="text-2xl text-gray-300">→</b> : null}</div>; })}</div>
