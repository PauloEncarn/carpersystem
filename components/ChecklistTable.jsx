...
export function ChecklistTable({ documentName = "RG.QUA.005", loteId = "", registro, subregistro, groups = checklistGroups, onSave, stepByStep = false }) {
  ...
  if (stepByStep) {
    ...
    return <section className="mx-auto max-w-3xl overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm"><header className="border-b border-gray-200 bg-gray-50 p-4 md:p-5"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[.16em] text-cicopal-blue">{row?.group}</p><h2 className="mt-1 text-xl font-black text-gray-950">Higienização · RG 003</h2></div><span className="rounded-full bg-white px-3 py-2 text-sm font-black text-gray-600">{activeIndex + 1}/{rows.length}</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-gray-200"><div className="h-full rounded-full bg-cicopal-blue transition-all" style={{ width: `${progress}%` }} /></div><p className="mt-2 text-xs font-bold text-gray-500">Item {groupPosition} de {groupRows.length} nesta seção</p></header>
    <div className="p-4 md:p-6"><p className="text-sm font-bold uppercase text-gray-400">Verifique o item</p><h3 className="mt-2 min-h-20 text-2xl font-black leading-tight text-gray-950 md:text-3xl">{row?.item}</h3><div className="mt-5 grid grid-cols-2 gap-3"><button type="button" className={`min-h-24 rounded-2xl border-2 p-3 text-lg font-black ${row?.av1 === "C" ? "border-cicopal-green bg-cicopal-green text-white" : "border-green-200 bg-green-50 text-cicopal-green"}`} onClick={() => updateRow(activeIndex, { av1: "C" })}><CheckCircle2 size={28} className="mx-auto mb-2" />Conforme</button><button type="button" className={`min-h-24 rounded-2xl border-2 p-3 text-lg font-black ${row?.av1 === "NC" ? "border-cicopal-red bg-cicopal-red text-white" : "border-red-200 bg-red-50 text-cicopal-red"}`} onClick={() => updateRow(activeIndex, { av1: "NC" })}><XCircle size={28} className="mx-auto mb-2" />Não conforme</button></div>
