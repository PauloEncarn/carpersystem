...
import { AlertTriangle, CheckCircle2, Clock3, Factory, Gauge, Radio, Thermometer, Users, X } from "lucide-react";

const plantLines = [
  {
    id: "PUR", name: "Pururuca", short: "PUR", product: "Pururuca Original 90g", status: "running", x: 120, y: 125, w: 235, h: 145,
    operator: "Carlos Mendes", shift: "A", completion: 92, ncs: 0, temp: "178 °C", cadence: "2.840 pct/h", lastOffset: 3,
  ...
  },
  {
    ...
    operator: "Marina Souza", shift: "A", completion: 78, ncs: 2, temp: "171 °C", cadence: "3.120 pct/h", lastOffset: 8,
  ...
  },
  {
    ...
    operator: "João Ribeiro", shift: "A", completion: 61, ncs: 1, temp: "—", cadence: "Parada técnica", lastOffset: 16,
...
export function FactorySupervision() {
  ...
  const totalNcs = useMemo(() => plantLines.reduce((total, line) => total + line.ncs, 0), []);
  ...
  return (
    <div className="space-y-4">
      <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="modern-panel flex items-center gap-3 p-4"><span className="grid size-11 place-items-center rounded-xl bg-green-50 text-cicopal-green"><Radio size={22} /></span><div><p className="text-xs font-bold uppercase text-gray-400">Linhas ativas</p><p className="text-xl font-black text-gray-950">2 de 3</p><p className="text-xs font-semibold text-cicopal-green">Monitoramento conectado</p></div></div>
      ...
      <section className="modern-panel overflow-hidden">
        ...
        <div className="relative overflow-x-auto bg-[#f0f2f6] p-3 md:p-5">
          <svg viewBox="0 0 1100 520" className="min-w-[850px] w-full" aria-label="Mapa interativo da fábrica">
            ...
            <rect x="20" y="20" width="1060" height="480" rx="28" fill="url(#floor-grid)" stroke="#cfd3dc" strokeWidth="2" />
            <rect x="55" y="55" width="990" height="55" rx="14" fill="#ffffff" stroke="#dfe3ec" />
            <text x="80" y="88" fill="#4f5666" fontSize="15" fontWeight="800">RECEBIMENTO E PREPARO</text><text x="1015" y="88" textAnchor="end" fill="#89909f" fontSize="11" fontWeight="700">MATÉRIA-PRIMA → PROCESSO → EMBALAGEM</text>
            <path d="M 355 197 L 400 197" stroke="#aeb4c1" strokeWidth="3" markerEnd="url(#flow-arrow)" /><path d="M 670 197 L 720 197" stroke="#aeb4c1" strokeWidth="3" markerEnd="url(#flow-arrow)" />
            {plantLines.map((line) => <PlantLine key={line.id} line={line} active={selectedLineId === line.id} hovered={hoveredLineId === line.id} onHover={setHoveredLineId} onSelect={setSelectedLineId} now={now} />)}
            <rect x="120" y="330" width="550" height="115" rx="18" fill="#fff" stroke="#dfe3ec" strokeWidth="2" /><text x="145" y="362" fill="#202532" fontSize="15" fontWeight="800">EMBALAGEM E INSPEÇÃO FINAL</text><rect x="145" y="380" width="145" height="40" rx="10" fill="#f4f5f8" /><text x="217" y="405" textAnchor="middle" fill="#606777" fontSize="11" fontWeight="700">EMPACOTADORAS</text><rect x="310" y="380" width="145" height="40" rx="10" fill="#f4f5f8" /><text x="382" y="405" textAnchor="middle" fill="#606777" fontSize="11" fontWeight="700">CHECKWEIGHER</text><rect x="475" y="380" width="165" height="40" rx="10" fill="#f4f5f8" /><text x="557" y="405" textAnchor="middle" fill="#606777" fontSize="11" fontWeight="700">PALETIZAÇÃO</text>
            <rect x="730" y="330" width="250" height="115" rx="18" fill="#1e22a8" /><text x="755" y="363" fill="white" fontSize="15" fontWeight="800">EXPEDIÇÃO</text><text x="755" y="391" fill="rgba(255,255,255,.72)" fontSize="12" fontWeight="600">12 lotes liberados hoje</text><text x="755" y="418" fill="white" fontSize="20" fontWeight="800">98,7% conformidade</text>
          </svg>
