...
} from "@/components/ChunkReloadGuard";

export default function Error({ error, reset }) {
  const chunkFailure = isChunkLoadFailure(error);

  useEffect(() => {
    if (chunkFailure) recoverFromChunkFailure(error);
  }, [chunkFailure, error]);
    ...
    <main className="flex min-h-screen items-center justify-center bg-cicopal-surface px-4">
      <section className="w-full max-w-xl rounded-md border border-red-200 bg-white p-5 shadow-soft">
        ...
        <p className="mt-2 text-sm font-semibold text-gray-600">
          {chunkFailure
            ? "Há uma atualização do sistema disponível. Estamos carregando a versão mais recente."
            : "Atualize a página. Se repetir, envie uma foto desta mensagem para identificarmos o ponto exato."}
        </p>
        ...
        <button
          ...
          onClick={() =>
            chunkFailure
              ? recoverFromChunkFailure(error) || window.location.reload()
              : reset()
          }
        >
          {chunkFailure ? "Carregar atualização" : "Tentar novamente"}
        </button>
      </section>
    </main>
  );
