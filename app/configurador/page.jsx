...
export default function ConfiguradorPage() {
  ...
  const canAccess = loggedUser?.permissoes?.includes("configurador:acessar") || loggedUser?.permissoes?.includes("admin:acessar");
    ...
    return (
      <main className="modern-ui grid min-h-screen place-items-center px-4">
        <section className="modern-panel max-w-lg p-8 text-center">
          <span className="mx-auto grid size-16 place-items-center rounded-2xl bg-red-50 text-cicopal-red"><ShieldCheck size={30} /></span>
          <h1 className="mt-5 text-2xl font-black text-gray-950">Acesso não autorizado</h1>
          <p className="mt-2 font-semibold text-gray-500">Seu perfil não possui permissão para acessar o Configurador de RG.</p>
          <Link href="/" className="mt-6 inline-flex min-h-12 items-center gap-2 rounded-xl bg-cicopal-blue px-5 font-bold text-white"><ArrowLeft size={18} /> Voltar para operação</Link>
        </section>
      </main>
    );
  }
      ...
      <header className="app-topbar sticky top-0 z-20 shadow-sm">
        <div className="mx-auto flex max-w-[1500px] flex-wrap items-center justify-between gap-3 px-4 py-3">
          <div className="flex items-center gap-3">
            <span className="app-brand-mark"><FileCog size={23} /></span>
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-cicopal-red">CICOPAL</p>
              <h1 className="text-lg font-black tracking-tight text-cicopal-blue">Configurador de RG</h1>
            </div>
          </div>
