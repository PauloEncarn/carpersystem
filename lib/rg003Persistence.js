...
export async function uploadOperationalPhoto({ image, cycle, category, hour = "", fileName = "evidencia.jpg" }) {
...
export async function persistChecklistCycleNcs({ cycle, operatorId, operatorName, processType, ncs }) {
  ...
  const rows = await Promise.all(freshNcs.map(async (nc) => ({
    ...
    aberta_em: new Date().toISOString(),
    metadata: { operador_nome: operatorName, grupo: nc.grupo ?? null, horario: nc.horario ?? "", rodada_id: nc.rodadaId ?? null, numero_rodada: nc.rodada ?? null, bloqueante: nc.bloqueante !== false },
  })));
...
export async function inspectHygieneRound({ round, operatorId, payload }) {
  ...
  const hasBlockingNc = (payload.ncs ?? []).some(
    (nc) => nc.bloqueante !== false,
  );
  const { data, error } = await supabase.rpc("inspecionar_higienizacao", {
    ...
    p_status: hasBlockingNc ? "em_correcao" : "aprovada",
    p_operador_id: safeOperatorId,
  });
  if (error) throw error;
  await supabase.from("eventos_ciclo").insert({
    ciclo_id: round.ciclo_id,
    tipo: hasBlockingNc ? "higienizacao_reprovada" : "higienizacao_aprovada",
    descricao: `${hasBlockingNc ? "Higienização reprovada" : hasNc ? "Higienização aprovada com NC não bloqueante" : "Higienização aprovada"} pela Qualidade · rodada ${round.numero}`,
    operador_id: safeOperatorId,
    dados: { rodada_id: round.id, numero_rodada: round.numero, nao_conformidades: payload.ncs?.length ?? 0, nao_conformidades_bloqueantes: (payload.ncs ?? []).filter((nc) => nc.bloqueante !== false).length },
  });
  return data;
}
...
export async function registerCycleShiftHandover({ cycleId, operatorId, fromShift, toShift, responsibleName = "", observation = "" }) {
  if (!isSupabaseConfigured || !supabase || !cycleId || !/^[0-9a-f-]{36}$/i.test(cycleId)) {
    return { id: `local-shift-${Date.now()}`, iniciado_em: new Date().toISOString() };
  }
  const resolvedOperatorId = await resolveOperatorId(operatorId);
  const { data, error } = await supabase.rpc("registrar_passagem_turno", {
    p_ciclo_id: cycleId,
    p_operador_id: resolvedOperatorId,
    p_turno_origem: fromShift || null,
    p_turno_destino: toShift,
    p_responsavel_nome: responsibleName || null,
