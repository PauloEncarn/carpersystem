...
function normalizeCycle(row, events = []) {
  ...
  return {
    ...
    productionCode:
      ...
      makeRg003ProductionCode(row.produto, row.iniciado_em),
    product: row.produto,
...
export async function startRg003Cycle({ lineId, product, reason, operatorId }) {
  ...
  const { data: coded, error: codeError } = await supabase
    .from("ciclos_producao")
    .update({ metadata: { ...(data.metadata ?? {}), productionCode } })
    .eq("id", data.id)
    .select()
    .single();
...
export async function persistCycleNc({
  ...
  const { data: nc, error } = await supabase
    ...
    .insert({
      ...
      metadata: { operador_nome: operatorName },
    })
    .select()
    .single();
  if (error) throw error;
...
export async function persistRg003Record({
  ...
  const [{ data: rg, error: rgError }, { data: process, error: processError }] =
    await Promise.all([
      ...
      supabase
        ...
        .select("id,rg_id")
        .eq("tipo", processType)
        .eq("rg_id", "00000000-0000-0000-0000-000000000303")
        .single(),
    ]);
