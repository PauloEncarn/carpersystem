...
export async function persistRg003Record({ lineId, loteCode, recordCode, processType, operatorId, turno, registro, subregistro, cycleId, cycleStartedAt, productionCode }) {
  ...
  const { data: existingLote, error: existingLoteError } = await supabase.from("lotes").select("*").eq("linha_id", lineId).eq("data_producao", productionDate).maybeSingle();
  if (existingLoteError) throw existingLoteError;
  let lote = existingLote;
  if (!lote) {
    const dailyCode = `${lineId}-${productionDate.replaceAll("-", "")}`;
    const { data: createdLote, error: loteError } = await supabase.from("lotes").insert({ codigo: dailyCode, linha_id: lineId, data_producao: productionDate, produto: "Produções do dia", metadata: { container_dia: true } }).select().single();
  ...
  const requestedHour = subregistro.apontamentos?.[0]?.horario ?? subregistro.avaliacoes?.[0]?.horario ?? "";
  const reportedHour = /^([01]\d|2[0-3]):[0-5]\d$/.test(requestedHour) ? requestedHour : hour.slice(0, 5);
  const slotKey = ["higienizacao", "produto_liberacao"].includes(processType) ? "ciclo" : subregistro._slotKey ?? `${productionDate}T${reportedHour}`;
...
export async function loadRg003Record(recordCode) {
  ...
  const fillings = [...record.preenchimentos].sort((a, b) => new Date(a.preenchido_em) - new Date(b.preenchido_em)).map((item) => ({ fillingId: item.id, subregistro: item.valores, filledAt: item.preenchido_em, version: item.versao, operatorId: item.operador_id }));
  const latest = fillings[fillings.length - 1];
  return { record, fillings, ...latest };
}
