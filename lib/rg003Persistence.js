...
export async function persistCycleTransition({ cycle, status, description, operatorId, operatorName, activeAction }) {
  ...
  const { error: updateError } = await supabase.from("ciclos_producao").update({ status: statusToDb[status] ?? status, etapa_iniciada_em: occurredAt, metadata }).eq("id", cycle.id);
...
export async function persistRg003Record({ lineId, loteCode, recordCode, processType, operatorId, turno, registro, subregistro, cycleId }) {
  ...
  const { data: filling, error: fillingError } = await supabase.from("preenchimentos").insert({ registro_id: savedRecord.id, processo_id: process.id, operador_id: operatorId || null, contexto_tipo: processType, horario: hour, valores: subregistro, status: subregistro.ncs?.length ? "com_nc" : "gravado" }).select().single();
