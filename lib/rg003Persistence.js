...
export async function persistCycleTransition({
  ...
  if (status === "producing" && !cycle.productionStartedAt)
    updatePayload.producao_iniciada_em = occurredAt;
  if (status === "ended") {
...
export async function persistCycleNc({
  ...
  const { data: nc, error } = await supabase
    ...
    .insert({
      ...
      foto_antes: data.fotoAntes ?? null,
...
export async function persistChecklistCycleNcs({ cycle, operatorId, operatorName, processType, ncs }) {
  ...
  const rows = ncs.map((nc) => ({
    ...
    item: nc.item,
    foto_antes: nc.fotoAntes ?? null,
    aberta_em: new Date().toISOString(),
    metadata: { operador_nome: operatorName, grupo: nc.grupo ?? null },
  }));
...
export async function startCyclePause({ cycleId, operatorId, reason, photoBefore }) {
  ...
  const safeOperatorId = await resolveOperatorId(operatorId);
  const { data, error } = await supabase.from("ciclo_pausas").insert({ ciclo_id: cycleId, motivo: reason, iniciada_por: safeOperatorId, foto_antes: photoBefore ?? null }).select().single();
  if (error) throw error;
  return data;
}
...
export async function finishCyclePause({ pauseId, operatorId, observation, photoAfter }) {
  ...
  const safeOperatorId = await resolveOperatorId(operatorId);
  const { data, error } = await supabase.from("ciclo_pausas").update({ encerrada_em: new Date().toISOString(), encerrada_por: safeOperatorId, observacao_retomada: observation, foto_depois: photoAfter ?? null }).eq("id", pauseId).select().single();
  if (error) throw error;
  return data;
}
...
export async function persistRg003Record({
  ...
  const requestedHour =
    subregistro.apontamentos?.[0]?.horario ??
    subregistro.avaliacoes?.[0]?.horario ??
    "";
