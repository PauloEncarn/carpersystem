...
export async function sendDailyEmailReport(now = new Date()) {
  ...
  const elapsedHours = Math.max(1, Math.ceil((end - start) / 3_600_000));
  const expectedControls = producingInWindow.length * elapsedHours * 3;
  const pendingControls = Math.max(0, expectedControls - hourlyFillings.length);
  const summary = `<table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-bottom:18px;background:#fff;border:1px solid #dfe3eb"><tr><td colspan="4" style="padding:14px 16px;border-bottom:1px solid #e6e9ef"><strong style="font-size:15px;color:#202476">Visão geral do dia</strong></td></tr><tr><td width="25%" align="center" style="padding:14px 5px;border-right:1px solid #e6e9ef"><strong style="display:block;font-size:24px;color:#202476">${activeCycles}</strong><span style="font-size:10px;color:#687080">PRODUÇÕES ATIVAS</span></td><td width="25%" align="center" style="padding:14px 5px;border-right:1px solid #e6e9ef"><strong style="display:block;font-size:24px;color:#087443">${hourlyFillings.length}</strong><span style="font-size:10px;color:#687080">REGISTROS NO DIA</span></td><td width="25%" align="center" style="padding:14px 5px;border-right:1px solid #e6e9ef"><strong style="display:block;font-size:24px;color:${pendingControls ? "#986500" : "#087443"}">${pendingControls}</strong><span style="font-size:10px;color:#687080">CONTROLES PENDENTES</span></td><td width="25%" align="center" style="padding:14px 5px"><strong style="display:block;font-size:24px;color:${openNcs ? "#c50d1d" : "#087443"}">${openNcs}</strong><span style="font-size:10px;color:#687080">NCS ABERTAS</span></td></tr></table>`;
...
// Compatibilidade temporária com integrações que ainda usam o nome anterior.
export const sendHourlyEmailReport = sendDailyEmailReport;
