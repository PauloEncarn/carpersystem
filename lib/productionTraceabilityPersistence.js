...
export async function createBatch({ cycleId, recipeId, inputs, userId }) {
  ...
  const rows = inputs.map((input) => ({
    batelada_id: batch.id,
    insumo_id: input.supplyId,
    automacao_lote_id: input.automationLotId || null,
    lote_fornecedor: input.lot,
    fornecedor: input.supplier || null,
    validade: input.expiry || null,
    quantidade_prevista: input.expected || null,
    quantidade_utilizada: input.used || null,
    unidade: input.unit || "kg",
    origem: input.origin,
    operador_id: user,
  }));
  const { error: inputError } = await supabase
    .from("batelada_insumos")
    .insert(rows);
...
export async function finishBatch(batchId, userId) {
  const { data, error } = await supabase
    ...
    .update({
      ...
      finalizada_por: await operatorId(userId),
    })
    .eq("id", batchId)
...
export async function startBatchConsumption(batchId, cycleId, userId) {
  const user = await operatorId(userId);
  const now = new Date().toISOString();
  const { error: previousError } = await supabase.from("bateladas").update({ status: "consumida", consumida_em: now, consumida_por: user }).eq("ciclo_id", cycleId).eq("status", "em_consumo");
  if (previousError) throw previousError;
  const { data, error } = await supabase.from("bateladas").update({ status: "em_consumo", consumo_iniciado_em: now, consumo_iniciado_por: user }).eq("id", batchId).eq("status", "pronta").select().single();
  if (error) throw error;
  return data;
}
export async function finishActiveBatch(cycleId, userId) {
  ...
  const now = new Date().toISOString();
  const user = await operatorId(userId);
  const { data, error } = await supabase.from("bateladas").update({ status: "consumida", consumida_em: now, consumida_por: user }).eq("ciclo_id", cycleId).in("status", ["em_preparacao", "pronta", "em_consumo"]).select();
  if (error) throw error;
  return data ?? [];
}
