...
const sampleWeights = Array.from({ length: 10 }, (_, index) => ({
  key: `peso_${index + 1}`,
  label: `Peso do produto ${index + 1}`,
  unit: "g",
}));
...
export const ROSCA_SUBPROCESSES = [
  ...
  },
  {
    ...
    equipment: "Antes do empacotamento",
    frequency: "hourly",
    parameters: [
      {
        key: "ferroso",
        label: "Corpo de prova ferroso",
        type: "options",
        options: ["C", "NC", "NA"],
      },
      {
        key: "nao_ferroso",
        label: "Corpo de prova não ferroso",
        type: "options",
        options: ["C", "NC", "NA"],
      },
      {
        key: "inox",
        label: "Corpo de prova inox",
        type: "options",
        options: ["C", "NC", "NA"],
      },
    ],
...
export async function changeSubprocessState({
  ...
  const { data, error } = await supabase.rpc("alterar_estado_subprocesso", {
    ...
    p_versao_esperada: process.versao,
    p_status: status,
