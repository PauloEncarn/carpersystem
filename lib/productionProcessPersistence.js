...
const sampleWeights = Array.from({ length: 10 }, (_, index) => ({ key: `peso_${index + 1}`, label: `Peso do produto ${index + 1}`, unit: "g" }));
const packerReadings = Array.from({ length: 4 }, (_, index) => [
  { key: `maq_${index + 1}_velocidade_esteira`, label: `Máquina ${index + 1} · Velocidade esteira`, unit: "m/min" },
  { key: `maq_${index + 1}_pacotes_min`, label: `Máquina ${index + 1} · Pacotes`, unit: "pacotes/min" },
  { key: `maq_${index + 1}_sobrepeso`, label: `Máquina ${index + 1} · Sobrepeso`, unit: "g" },
]).flat();
...
export const ROSCA_SUBPROCESSES = [
  ...
  { code: "corte_fio", name: "Corte a fio", equipment: "Cortadora", frequency: "hourly", parameters: [...sampleWeights, { key: "umidade", label: "Umidade", unit: "%" }, { key: "cortes_hora", label: "Cortes por hora", unit: "cortes/h" }], liveMetrics: [{ key: "cortes_hora", label: "Cortes projetados", unit: "cortes/min", divisor: 60 }] },
  { code: "forno", name: "Forno", equipment: "Forno · 7 zonas", frequency: "hourly", parameters: [{ key: "velocidade_esteira", label: "Velocidade da esteira", unit: "m/min" }, { key: "velocidade_linha", label: "Velocidade da linha", unit: "m/min" }, { key: "umidade", label: "Umidade", unit: "%" }, ...zones], liveMetrics: [{ key: "umidade", label: "Umidade", unit: "%" }, { key: "velocidade_linha", label: "Velocidade da linha", unit: "m/min" }] },
  { code: "detector_metal", name: "Detecção de metais", equipment: "Antes do empacotamento", frequency: "hourly", parameters: [{ key: "ferroso", label: "Corpo de prova ferroso", type: "options", options: ["C","NC","NA"] }, { key: "nao_ferroso", label: "Corpo de prova não ferroso", type: "options", options: ["C","NC","NA"] }, { key: "inox", label: "Corpo de prova inox", type: "options", options: ["C","NC","NA"] }] },
  { code: "empacotamento", name: "Empacotamento", equipment: "4 empacotadoras", frequency: "hourly", parameters: packerReadings, liveMetrics: [{ key: "maq_1_pacotes_min", label: "Máquina 1", unit: "pacotes/min" }, { key: "maq_2_pacotes_min", label: "Máquina 2", unit: "pacotes/min" }] },
  { code: "encaixotamento", name: "Encaixotamento", equipment: "Final de linha", frequency: "hourly", parameters: [{ key: "caixas_min", label: "Caixas por minuto", unit: "caixas/min" }], liveMetrics: [{ key: "caixas_min", label: "Caixas", unit: "caixas/min" }] },
];
