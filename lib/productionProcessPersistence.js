...
import { uploadOperationalPhoto } from "@/lib/rg003Persistence";
...
export async function reportSubprocessProblem({
  ...
  const uploadedPhoto = await uploadOperationalPhoto({
    image: photoBefore,
    cycle: cycleId,
    category: "problema_antes",
    fileName: `problema-${processId}-antes.jpg`,
  });
  const { data, error } = await supabase.rpc("registrar_problema_subprocesso", {
    p_subprocesso_id: processId,
    p_equipamento: equipment,
    p_causa: cause,
    p_descricao: description || "",
    p_foto_antes: uploadedPhoto,
    p_operador_id: await safeOperator(operatorId),
  });
  if (error) throw error;
  return data;
...
export async function resolveSubprocessProblem({
  ...
  const uploadedPhoto = await uploadOperationalPhoto({
    image: photoAfter,
    cycle: cycleId,
    category: "problema_depois",
    fileName: `problema-${problemId}-depois.jpg`,
  });
  const { data, error } = await supabase.rpc("resolver_problema_subprocesso", {
    p_problema_id: problemId,
    p_solucao: resolution,
    p_foto_depois: uploadedPhoto,
    p_operador_id: await safeOperator(operatorId),
  });
  if (error) throw error;
  return data;
